...
public class HiveUtil {
    ...
    public HiveUtil(){
        try{
            ...
            String url = jdbcOperator.getClusterEnv("hive");
            //1、数据仓库连接
            conn = DriverManager.getConnection(url,"dmp","");
            statement = conn.createStatement();
        } catch (SQLException | EnvParamsException e) {
            e.printStackTrace();
        }
    }

    public void  setSparkAppName(String appName){
        Statement statement = null;
        try {
            statement = conn.createStatement();
            statement.execute("set spark.app.name=" + appName);
