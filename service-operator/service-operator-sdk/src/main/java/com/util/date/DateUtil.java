...
public class DateUtil implements Serializable {
    ...
    private static final HashMap<String, SimpleDateFormat> sdMap = new HashMap<>();
    ...
    private static SimpleDateFormat getDateFormat(String pattern)
        ...
        SimpleDateFormat dateFormat = sdMap.get(pattern);
        if (dateFormat == null) {
            synchronized (object) {
                dateFormat = new SimpleDateFormat(pattern);
                dateFormat.setLenient(false);
                //threadLocal.set(dateFormat);
                sdMap.put(pattern, dateFormat);
            }
        }
    ...
    public static String getDateString(Long date) {
        return DateToString(new Date(date), DateStyle.YYYY_MM_DD_HH_MM_SS_SSS);
    }
    ...
    public static String getDateTimeMilli(Date date) {
        return DateToString(date, DateStyle.YYYY_MM_DD_HH_MM_SS_SSS);
    }
    ...
    public static String getYesterday(DateStyle dateStyle) {
        Date date = addDay(new Date(), -1);
        String yesterday = DateToString(date, dateStyle);
        return yesterday;
    }

    public static String getNowDateSubtractThreeDay(DateStyle dateStyle) {
        Date date = addDay(new Date(), -3);
        String subtractThreeDay = DateToString(date, dateStyle);
        return subtractThreeDay;
    }
    ...
    public static String getDateTime(Long time) {
        return DateToString(new Date(time), DateStyle.YYYY_MM_DD_HH_MM_SS);
    ...
    //根据出生年月日yyyy-MM-dd计算年龄
    //跟历史数据补充保持一直，小于1970-00-00或者大于当前时间为0 不满一岁为0  其余正常
    public static int getAge(String birthDay, DateStyle dateStyle) {
        Date date = StringToDate(birthDay, dateStyle);
        if ((new Date()).before(date) || StringToDate("1970-01-01", DateStyle.YYYY_MM_DD).after(date)) { //出生日期晚于当前时间
            return 0;
        }
